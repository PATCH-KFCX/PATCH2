...
exports.seed = async (knex) => {
  ...
  const users = await Promise.all(
    names.map((name, i) => User.create(name, age[i], emails[i], 'password123')),
  ...
  // Step 5: Seed medications
  await knex('medications').del();
  await knex.raw('ALTER SEQUENCE medications_id_seq RESTART WITH 1');

  const medications = [
    { name: 'Metformin', dosage: '500mg', days: 'Mon,Tue,Wed,Thu,Fri,Sat,Sun' },
    { name: 'Ibuprofen', dosage: '200mg', days: 'Mon,Wed,Fri' },
    { name: 'Lisinopril', dosage: '10mg', days: 'Mon,Tue,Wed,Thu,Fri,Sat,Sun' },
  ];

  await knex('medications').insert(medications);

  // Step 6: Get the freshly inserted users and meds
  const allUsers = await knex('users').select('id');
  const allMeds = await knex('medications').select('id');

  // Step 7: Seed user_medications
  ...
  const userMedications = allUsers.map((user, i) => ({
    user_id: user.id,
    medication_id: allMeds[i % allMeds.length].id,
    start_date: today,
    end_date: null,
